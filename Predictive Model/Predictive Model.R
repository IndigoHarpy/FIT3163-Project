predict_result <- function (player_1, player_2, surface, tourney_level, round, match_dur, matches, players) {
  ...
  match_data_1 <- data.frame("surface" = surface, "tourney_level" = tourney_level, "opp_hand" = player_2$hand, "round" = round, "minutes" = match_dur, "ht_diff" = player_1$height - player_2$height, "age_diff" = as.numeric(player_1$dob - player_2$dob)/365, "rank_diff" = player_1$Rating - player_2$Rating)
  
  match_data_2 <- data.frame("surface" = surface, "tourney_level" = tourney_level, "opp_hand" = player_1$hand, "round" = round, "minutes" = match_dur, "ht_diff" = player_2$height - player_1$height, "age_diff" = as.numeric(player_2$dob - player_1$dob)/365, "rank_diff" = player_2$Rating - player_1$Rating)
...
get_matches <- function(player, matches, players) {
  # Creates empty data frame for matches they have played in
